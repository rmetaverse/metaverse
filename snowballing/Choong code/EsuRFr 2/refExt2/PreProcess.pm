...
#$#use Omni::Config;
use refExt2::Citation;

my %marker_types =	(	'SQUARE'		=> '\\[.+?\\]',
		   				'PAREN'			=> '\\(.+?\\)',
		   				'NAKEDNUM'		=> '\\d{1,3}\\s',#'NAKEDNUM'		=> '\\d+', # MKCHOONG
			   			'NAKEDNUMDOT'	=> '\\d{1,3}\\.\\s',#'NAKEDNUMDOT'	=> '\\d+\\.', # MKCHOONG
                        #'COCHRANE'      => '\\w+\\s\\w+\\s\\d{4}[abc]*\\s\\{.+?\\}|\\w+\\s\\d{4}[abc]*\\s\\{.+?\\}|\\w+\\s\\w+\\s\\d{4}[abc]*|\\w+\\s\\d{4}[abc]*', # Modified by MKChoong June 2013
                        #'\\w+\\s|\\w+\\s\\w+\\s\\d{4}[abc]*\\s?\\{?.+?\\}?'
                        #'\\w+\\s\\d{4}[abc]*\\s\\{.+?\\}'
                        #'\\w+\\s\\w+\\s\\d{4}[abc]*\\s\\{.+?\\}'
                        #'\\w+\\s\\w+\\s\\d{4}[abc]*'
                        #'\\w+\\s\\d{4}[abc]*'
		...
		for (my $y = $start_column; $y < scalar(@{ $columns }); $y++)
		{
			...
			for (my $z = $start_para; $z < scalar(@{ $paras }); $z++)
			{
				...
				for (my $t = $start_line; $t < scalar(@{ $lines }); $t++)
				{
					...
					my $tmp = undef;
					# Is it the end?					
					#mkchoongif ($ln_content =~ m/^([\s\d\.]+)?(Acknowledge?ments?|Autobiographical|Tables?|Appendix|Exhibit|Annex|Fig|Notes?)(.*?)$/)
                    if ($ln_content =~ m/^([\s\d\.]+)?(Acknowledge?ments?|Autobiographical|Appendix)(.*?)$/) #mkchoong28aug13
					{
...
{    
	...
	# {
	###
    while ($text =~ m/\b(References?|REFERENCES?|Bibliography|BIBLIOGRAPHY|References?\s+and\s+Notes?|References?\s+Cited|REFERENCES?\s+CITED|REFERENCES?\s+AND\s+NOTES?|LITERATURE?\s+CITED?|References?\s+and\s+recommended\s+reading):?\s*\W*\n+/sg) 
	{
	...
	if ($citetext eq "")
	{
        ...
        return \$citetext, \$bodytext, \$bodytext;
	}
    
#	# Odd case: when citation is longer than the content itself, what should we do?
#    if (length($citetext) >= 0.5 * length($bodytext)) #if (length($citetext) >= 0.8 * length($bodytext)) 
#	{
#		print STDERR "Citation text longer than article body: ignoring\n";
#		return \$citetext, NormalizeBodyText(\$bodytext, $pos_array), \$bodytext;
#    }
    ...
    #mkchoong my ($scitetext, $tmp) = split(/^([\s\d\.]+)?(Acknowledge?ments?|Autobiographical|Tables?|Appendix|Exhibit|Annex|Fig|Notes?|Executive\s+summary)(.*?)\n+/m, $citetext);    
    my ($scitetext, $tmp) = split(/^([\s\d\.]+)?(Acknowledge?ments?|Autobiographical|Appendix|Executive\s+summary)(.*?)\n+/m, $citetext);    #mkchoong28aug13
    
	if (length($scitetext) > 0) { $citetext = $scitetext; }
    #if (length($scitetext) > 0) { print "truncated\n"; }
    #print "Length of citetext: ", length($citetext), "\n";
    #print "Length of bodytext: ", length($bodytext), "\n";
    
	# Odd case: when citation is longer than the content itself, what should we do?
    if (length($citetext) >= 0.5 * (length($bodytext)+length($citetext))) #if (length($citetext) >= 0.8 * length($bodytext)) 
	{
		print STDERR "Citation text longer than article body: ignoring\n";
		return \$citetext, \$bodytext, \$bodytext;
    }
    ...
    return (NormalizeCiteText(\$citetext), \$bodytext, \$bodytext);
}
...
sub SplitCitationsByMarker 
{
    ...
    my @citations 				= ();
    my $current_citation		= new refExt2::Citation();
    ...
    foreach my $line (split "\n", $$rcite_text) 
	{
		if ($line =~ m/^\s*($marker_types{ $marker_type })\s*(.*)$/) 
		{
	    	...
	    	$current_citation			= new refExt2::Citation();
...
sub SplitUnmarkedCitations 
{
    ...
    for (my $k = 0; $k <= $#cite_starts; $k++) 
	{
		...
		my $citation	= new refExt2::Citation();
...
#$#				if ($cit_str ne "") 
#$#				{
#$#					my $citation = new refExt::Citation();
#$#
...
sub GuessMarkerType 
{
	...
	###
    while ($cite_text =~ m/\n\s*($marker_types{'NAKEDNUM'}([^\n]){10})/sg) #MKCHOONG#while ($cite_text =~ m/\n\s*($marker_types{'NAKEDNUM'} [^\n]{10})/sg) 
	{ 
    ...
    ###
    # Modified by MKChoong June 2013
    ###
    #while ($cite_text =~ m/\n\s*$marker_types{'COCHRANE'}/sg) #m/\n\s*$marker_types{'COCHRANE'}([^\n])/sg) 
	#{
	#	$marker_observations{'COCHRANE'}++;
    #}
    ...
    ###
    # Modified by MKChoong July 2013
    ###
    #if ($marker_type eq 'COCHRANE')
    #{
    #    if ($marker_observations { $sorted_observations[1] } >= $min_markers)
    #    {
    #        $marker_type = $sorted_observations[1];
    #    }
    #}
